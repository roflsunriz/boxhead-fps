...
bun run test         # Playwright E2E、86 項目すべて PASS すること
...
2. リロード時に同じマガジングループが抜去・非表示・再挿入される。
3. ボットのカービン、反動、マズルフラッシュ、味方アウトラインが追従する。
4. pickupと投擲グレネードの取得・物理・爆発判定がモデル置換後も変わらない。
5. カービンが4方向で平面へ潰れず、意図したstock／handguard穴が残る。
6. クレーターの焦げ中心、低い不規則縁、瓦礫が見え、円形トーラスのケーキ状外周へ戻っていない。
