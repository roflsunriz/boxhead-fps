...
export function createPalmTreeModel(height: number, seed: number): THREE.Group {
  ...
  const leaf = new THREE.MeshStandardMaterial({ color: 0x2f6f42, roughness: 0.78, side: THREE.DoubleSide });
  const random = (() => {
    let state = seed >>> 0;
  ...
  const curve = new THREE.CatmullRomCurve3([
    ...
    new THREE.Vector3((random() - 0.5) * 0.7, height * 0.35, (random() - 0.5) * 0.5),
    new THREE.Vector3((random() - 0.5) * 1.2, height * 0.7, (random() - 0.5) * 0.8),
    new THREE.Vector3((random() - 0.5) * 1.5, height, (random() - 0.5) * 1.1),
  ]);
  addMesh(
    group,
    "curved-trunk",
    new THREE.TubeGeometry(curve, 26, height * 0.034, 10, false),
    bark,
    [0, 0, 0]
  );
  for (let index = 1; index < 14; index++) {
    ...
    addMesh(
      group,
      "bark-ring",
      new THREE.TorusGeometry(height * 0.035, height * 0.005, 5, 14),
      bark,
  ...
  for (let index = 0; index < 11; index++) {
    const angle = (index / 11) * Math.PI * 2 + random() * 0.2;
    const length = height * (0.22 + random() * 0.08);
    const frondCurve = new THREE.QuadraticBezierCurve3(
      crown,
      crown
        .clone()
        .add(
          new THREE.Vector3(Math.cos(angle) * length * 0.55, height * 0.08, Math.sin(angle) * length * 0.55)
        ),
      crown
        .clone()
        .add(
    ...
    addMesh(
      frond,
      "frond-spine",
      new THREE.TubeGeometry(frondCurve, 12, height * 0.006, 5, false),
      leaf,
      [0, 0, 0]
    );
    for (let segment = 2; segment < 12; segment++) {
      const t = segment / 12;
      const point = frondCurve.getPoint(t);
      const tangent = frondCurve.getTangent(t);
      const side = new THREE.Vector3(-tangent.z, 0, tangent.x).normalize();
      const leafletLength = length * 0.15 * Math.sin(t * Math.PI);
      for (const sign of [-1, 1]) {
        const leaflet = addMesh(
          frond,
          "leaflet",
          new THREE.PlaneGeometry(leafletLength, height * 0.018),
          leaf,
          [point.x, point.y, point.z]
        );
        leaflet.lookAt(point.clone().add(side.multiplyScalar(sign)));
        leaflet.rotateX(-0.35);
        leaflet.userData.explodeWithParent = true;
      }
    }
  }
  const coconut = new THREE.MeshStandardMaterial({ color: 0x5b3924, roughness: 0.88 });
  for (let index = 0; index < 4; index++) {
    const angle = (index / 4) * Math.PI * 2;
    addMesh(group, "coconut", new THREE.SphereGeometry(height * 0.035, 12, 10), coconut, [
      crown.x + Math.cos(angle) * height * 0.045,
      crown.y - height * 0.04,
      crown.z + Math.sin(angle) * height * 0.045,
    ]);
  }
  return group;
}
