...
check("overlay hidden after start", await page.evaluate(() => document.getElementById("overlay").classList.contains("hidden")));
...
check(`player blocked by crate face z=1.5 + radius 0.5 (stopped at ${pInCrate.toFixed(2)})`, pInCrate > 1.4 && pInCrate < 3);
...
await page.evaluate(() => {
  const g = window.__game;
  g.enemies.forEach(en => { en.group.position.set(200, 0, 200); });
  g.player.pos.set(0, 1.7, 25);
...
const killResult = await page.evaluate(() => {
  ...
  g.enemies.forEach(en => { en.hp = 1; en.group.position.set(0, 0, 28); en.group.updateMatrixWorld(true); });
  const s0 = g.score;
  const n0 = g.enemies.length;
  g.tryShoot();
  return { ok: true, s0, s1: g.score, n0, n1: g.enemies.length };
});
check("raycast hits enemy and kills it", killResult.ok && killResult.n1 === killResult.n0 - 1,
      JSON.stringify(killResult));
check("score awarded on kill (+100)", killResult.s1 === killResult.s0 + 100,
      `${killResult.s0} -> ${killResult.s1}`);

console.log("\n[6] Reload");
await page.evaluate(() => { window.__game.enemies.forEach(en => en.group.position.set(200, 0, 200)); });
...
check(`reload refills magazine (${ra0} -> full)`, await page.evaluate(() => window.__game.ammo === 30 && !window.__game.reloading));
...
check("zero console/page errors during gameplay", consoleErrors.length === 0, JSON.stringify(consoleErrors.slice(0, 5)));
...
check("game loop runs in fallback mode", await page2.evaluate(() => window.__game.enemies.length >= 0 && !window.__game.gameOver));
...
if (colResult.ok) {
  ...
  check(`enemy blocked by center building (stayed at z=${ez.toFixed(2)}, did not cross to player side)`, ez < 0);
} else {
